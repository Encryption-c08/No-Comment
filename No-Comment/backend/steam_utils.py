"""Steam-related utilities used across NoComment backend modules."""
...
def detect_steam_install_path() -> str:
    ...
    logger.log(f"NoComment: Steam install path set to {_STEAM_INSTALL_PATH}")
...
def _find_steam_path() -> str:
    ...
    if sys.platform.startswith("win") and winreg:
        try:
            ...
            logger.warn(f"NoComment: Failed to read Steam path from registry: {exc}")

    return ""


def get_primary_steam_user_info() -> Dict[str, str]:
    """Return primary Steam user info from loginusers.vdf (steam_id, player_name)."""
    try:
        ...
        if not os.path.exists(login_path):
            logger.warn(f"NoComment: loginusers.vdf not found at {login_path}")
            return {}

        with open(login_path, "r", encoding="utf-8", errors="ignore") as handle:
            vdf_content = handle.read()
        vdf_data = _parse_vdf_simple(vdf_content)

        users = vdf_data.get("users") or vdf_data.get("Users") or {}
        if not isinstance(users, dict):
            logger.warn("NoComment: loginusers.vdf parsed without users")
            return {}

        def get_field(user_dict: Dict[str, any], key: str) -> Optional[str]:
        ...
        for steam_id, info in users.items():
            if not isinstance(info, dict):
                continue
            most_recent = get_field(info, "MostRecent")
            if str(most_recent) == "1":
                selected_id = str(steam_id)
                selected_user = info
                break
        ...
        if not selected_user or not selected_id:
            logger.warn("NoComment: No usable user entries found in loginusers.vdf")
            return {}

        persona = get_field(selected_user, "PersonaName") or get_field(selected_user, "AccountName") or ""
        return {"steam_id": selected_id, "player_name": str(persona)}
    except Exception as exc:
        logger.warn(f"NoComment: Failed to read loginusers.vdf: {exc}")
        return {}
...
def has_lua_for_app(appid: int) -> bool:
    try:
        ...
        logger.error(f"NoComment (steam_utils): Error checking Lua scripts for app {appid}: {exc}")
        return False
...
def get_game_install_path_response(appid: int) -> Dict[str, any]:
    ...
    if not os.path.exists(library_vdf_path):
        logger.warn(f"NoComment: libraryfolders.vdf not found at {library_vdf_path}")
    ...
    if not library_path:
        logger.log(
            f"NoComment: appid {appid} not in libraryfolders.vdf, searching all libraries for appmanifest"
        )
        for lib_path in all_library_paths:
            ...
            if os.path.exists(candidate_path):
                ...
                logger.log(f"NoComment: Found appmanifest at {appmanifest_path}")
                break
    else:
    ...
    if not library_path or not appmanifest_path or not os.path.exists(appmanifest_path):
        logger.log(f"NoComment: appmanifest not found for {appid} in any library")
    ...
    try:
        ...
        logger.warn(f"NoComment: Failed to parse appmanifest: {exc}")
        return {"success": False, "error": "Failed to parse appmanifest"}

    app_state = manifest_data.get("AppState", {})
    install_dir = app_state.get("installdir", "")
    if not install_dir:
        logger.warn(f"NoComment: installdir not found in appmanifest for {appid}")
    ...
    if not os.path.exists(full_install_path):
        logger.warn(f"NoComment: Game install path does not exist: {full_install_path}")
        return {"success": False, "error": "Game directory not found"}

    logger.log(f"NoComment: Game install path for {appid}: {full_install_path}")
    return {
        "success": True,
...
def open_game_folder(path: str) -> bool:
    ...
    try:
        ...
        logger.warn(f"NoComment: Failed to open game folder: {exc}")
        return False
