"""Management of the NoComment API manifest (free API list)."""
...
def fetch_free_apis_now(content_script_query: str = "") -> str:
    ...
    client = ensure_http_client("NoComment: FetchFreeApisNow")
    try:
        logger.log("NoComment: FetchFreeApisNow invoked")
        manifest_text = ""
        ...
        try:
            ...
            resp.raise_for_status()
            manifest_text = resp.text
            logger.log("NoComment: Fetched manifest from primary URL")
        except Exception as primary_err:
            logger.warn(f"NoComment: Primary manifest URL failed ({primary_err}), trying proxy...")
            try:
                resp = client.get(
                    API_MANIFEST_PROXY_URL,
                ...
                resp.raise_for_status()
                manifest_text = resp.text
                logger.log("NoComment: Fetched manifest from proxy URL")
            except Exception as proxy_err:
                logger.warn(f"NoComment: Proxy manifest URL also failed: {proxy_err}")
                return json.dumps(
        ...
        logger.warn(f"NoComment: FetchFreeApisNow failed: {exc}")
...
def load_api_manifest() -> List[Dict[str, Any]]:
    ...
    if normalized and normalized != text:
        try:
            ...
            logger.log("NoComment: Normalized api.json to valid JSON")
        except Exception:
            pass
        text = normalized
