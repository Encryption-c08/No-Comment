...
def _detect_steam_language() -> Optional[str]:
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Valve\Steam") as key:
            ...
            if locale_code:
                logger.log(
                    f"NoComment: detected Steam language '{steam_lang}' -> locale '{locale_code}'"
                )
                return locale_code
            logger.log(
                f"NoComment: Steam language '{steam_lang}' has no matching locale, using default"
            )
    except Exception as exc:
        logger.log(f"NoComment: could not read Steam language from registry: {exc}")
    return None
...
def _ensure_language_valid(values: Dict[str, Any]) -> bool:
    ...
    if current_language not in available_codes:
        logger.warn(
            f"NoComment: language '{current_language}' not available; "
...
def _ensure_settings_dir() -> None:
    ...
    try:
        ...
        logger.warn(f"NoComment: Failed to ensure settings directory: {exc}")
...
def _load_settings_file() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"NoComment: Failed to read settings file: {exc}")
        return {}
...
def _write_settings_file(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warn(f"NoComment: Failed to persist settings file: {exc}")
...
def _validate_option_value(option: SettingOption, value: Any) -> Tuple[bool, Any, str | None]:
    ...
    if option.option_type == "select":
        ...
        if dynamic == "locales":
            ...
            try:
                logger.log(
                    "NoComment: validating locale option "
            ...
            try:
                logger.warn(
                    f"NoComment: invalid locale selection {value!r}; allowed codes "
            ...
            try:
                logger.warn(
                    f"NoComment: invalid select option value {value!r}; allowed {sorted(allowed)}"
                )
            except Exception:
                pass
            return False, option.default, "Value not in list of allowed options"
    ...
    if first_launch:
        ...
        if detected:
            ...
            if detected in available_codes:
                ...
                logger.log(f"NoComment: first launch, auto-selected language '{detected}'")
            else:
                logger.log(
                    f"NoComment: detected locale '{detected}' not available "
...
def apply_settings_changes(changes: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with _SETTINGS_LOCK:
        ...
        for group_key, options_changes in changes.items():
            ...
            logger.log(f"NoComment: applying group {group_key} with payload {options_changes}")
            ...
            for option_key, value in options_changes.items():
                try:
                    logger.log(
                        f"NoComment: apply change request {group_key}.{option_key} -> {value!r}"
                    )
                except Exception:
                    pass
                ...
                try:
                    logger.log(
                        f"NoComment: validated {group_key}.{option_key}, "
        ...
        if not applied_changes and not language_changed:
            ...
            translations = get_locale_manager().get_locale_strings(language)
            logger.log(
                f"NoComment: no changes applied; returning cached values with language={language}"
            )
            return {
                "success": True,
                "values": values_snapshot,
        ...
        for option_key, previous, current_value in applied_changes:
            for callback in _CHANGE_HOOKS.get(option_key, []):
                try:
                    ...
                    logger.warn(f"NoComment: settings hook failed for {option_key}: {exc}")

        translations = get_locale_manager().get_locale_strings(language)

        logger.log(
            f"NoComment: apply_settings_changes final language={language}, values={values_snapshot}"
        )
        return {
            "success": True,
            "values": values_snapshot,
