(function() {
    ...
    function upsertStoreDlcManageButton(appId, dlcCount) {
        ...
        if (!btnContainer) {
            ...
            btnContainer.className = 'btn_addtocart btn_packageinfo NoComment-store-dlc-button-container';
        ...
        const button = btnContainer.querySelector('[data-NoComment-dlc-button="1"]');
        const buttonSpan = btnContainer.querySelector('[data-NoComment-dlc-button-label="1"]');
    ...
    function createStoreAddButton(appId) {
        ...
        btnContainer.className = 'btn_addtocart btn_packageinfo NoComment-store-button-container NoComment-store-add';
    ...
    function createStoreBundleAddButton() {
        ...
        const btnContainer = document.createElement('div');
        btnContainer.className = 'btn_addtocart btn_packageinfo NoComment-store-button-container NoComment-store-bundle';
    ...
    function createStoreRemoveButton(appId) {
        ...
        const btnContainer = document.createElement('div');
        btnContainer.className = 'btn_addtocart btn_packageinfo NoComment-store-button-container NoComment-store-remove';
    ...
    async function ensureStoreAddButton() {
        ...
        const existing = document.querySelector('.NoComment-store-button-container');
        if (isBundlePage()) {
            removeStoreDlcManageButton();
            dlcStoreCheckInFlight = false;
            dlcStoreCheckAppId = null;
    ...
    function ensureStyles() {
        if (!document.getElementById('NoComment-spacing-styles')) {
            ...
            style.textContent = '.NoComment-restart-button, .NoComment-button, .NoComment-icon-button{ margin-left:6px !important; }';
            document.head.appendChild(style); 
        }
    }
    ...
    function updateButtonTranslations() {
        try {
            ...
            const restartBtn = document.querySelector('.NoComment-restart-button');
            if (restartBtn) {
            ...
            const NoCommentBtn = document.querySelector('.NoComment-button');
            if (NoCommentBtn) {
                const addViaText = lt('Add via NoComment');
                NoCommentBtn.title = addViaText;
                NoCommentBtn.setAttribute('data-tooltip-text', addViaText);
                const span = NoCommentBtn.querySelector('span');
                if (span) {
    ...
    function addNoCommentButton() {
        ...
        try {
            const injected = document.querySelectorAll('.NoComment-restart-button, .NoComment-button, .NoComment-icon-button');
        ...
        if (steamdbContainer) {
            ...
            const existingBtn = document.querySelector('.NoComment-button');
            if (existingBtn) {
                ensureTranslationsLoaded(false).then(function() {
                    updateButtonTranslations();
                });
            ...
            if (existingBtn || window.__NoCommentButtonInserted) {
                if (!logState.existsOnce) { backendLog('NoComment button already exists, skipping'); logState.existsOnce = true; }
            ...
            try {
                if (!document.querySelector('.NoComment-restart-button') && !window.__NoCommentRestartInserted) {
                    ensureStyles();
                    const referenceBtn = steamdbContainer.querySelector('a');
                    const restartBtn = document.createElement('a');
                    if (referenceBtn && referenceBtn.className) {
                        restartBtn.className = referenceBtn.className + ' NoComment-restart-button';
                    } else {
                        restartBtn.className = 'btnv6_blue_hoverfade btn_medium NoComment-restart-button';
                    }
                    restartBtn.href = '#';
                    const restartText = lt('Restart Steam');
                    restartBtn.title = restartText;
                    ...
                    try {
                        if (!document.querySelector('.NoComment-icon-button') && !window.__NoCommentIconInserted) {
                            const iconBtn = document.createElement('a');
                            if (referenceBtn && referenceBtn.className) {
                                iconBtn.className = referenceBtn.className + ' NoComment-icon-button';
                            } else {
                                iconBtn.className = 'btnv6_blue_hoverfade btn_medium NoComment-icon-button';
                            }
                            iconBtn.href = '#';
                            iconBtn.title = 'NoComment Helper';
                            iconBtn.setAttribute('data-tooltip-text', 'NoComment Helper');
                            ...
                            try {
                                Millennium.callServerMethod('No-Comment', 'GetIconDataUrl', { contentScriptQuery: '' }).then(function(res){
                                    try {
                                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                        if (payload && payload.success && payload.dataUrl) {
                                            img.src = payload.dataUrl;
                                        } else {
                                            img.src = 'No-Comment/NoComment-icon.png';
                                        }
                                    } catch(_) { img.src = 'No-Comment/NoComment-icon.png'; }
                                });
                            } catch(_) {
                                img.src = 'No-Comment/NoComment-icon.png';
                            }
                            ...
                            window.__NoCommentIconInserted = true;
                            backendLog('Inserted Icon button');
                        }
                    } catch(_) {}
                    window.__NoCommentRestartInserted = true;
                    backendLog('Inserted Restart Steam button');
                }
            } catch(_) {}

            
            if (document.querySelector('.NoComment-button') || window.__NoCommentButtonInserted) {
                return;
            }
            
            
            let referenceBtn = steamdbContainer.querySelector('a');
            const NoCommentButton = document.createElement('a');
            NoCommentButton.href = '#';
            
            if (referenceBtn && referenceBtn.className) {
                NoCommentButton.className = referenceBtn.className + ' NoComment-button';
            } else {
                NoCommentButton.className = 'btnv6_blue_hoverfade btn_medium NoComment-button';
            }
            const span = document.createElement('span');
            const addViaText = lt('Add via NoComment');
            span.textContent = addViaText;
            NoCommentButton.appendChild(span);
            
            NoCommentButton.title = addViaText;
            NoCommentButton.setAttribute('data-tooltip-text', addViaText);
            
            try {
                if (referenceBtn) {
                    const cs = window.getComputedStyle(referenceBtn);
                    NoCommentButton.style.marginLeft = cs.marginLeft;
                    NoCommentButton.style.marginRight = cs.marginRight;
                }
            } catch(_) {}
            
            
            NoCommentButton.addEventListener('click', function(e) {
                e.preventDefault();
                ...
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    ...
                    Millennium.callServerMethod('No-Comment', 'HasNoCommentForApp', { appid, contentScriptQuery: '' }).then(function(res){
                        try {
                            ...
                            if (!document.querySelector('.NoComment-button') && !window.__NoCommentButtonInserted) {
                                const restartExisting = steamdbContainer.querySelector('.NoComment-restart-button');
                                if (restartExisting && restartExisting.after) {
                                    restartExisting.after(NoCommentButton);
                                } else if (referenceBtn && referenceBtn.after) {
                                    referenceBtn.after(NoCommentButton);
                                } else {
                                    steamdbContainer.appendChild(NoCommentButton);
                                }
            ...
            if (!logState.missingOnce) { backendLog('NoComment: steamdbContainer not found on this page'); logState.missingOnce = true; }
        }
    }
    ...
    setTimeout(addNoCommentButton, 1000);
    setTimeout(addNoCommentButton, 3000);
