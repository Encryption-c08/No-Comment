...
def _update_unsteam_ini(ini_full_path: str, appid: int, steam_id: Optional[str], player_name: Optional[str]) -> None:
    try:
        ...
        if updated_contents != contents:
            ...
            logger.log(
                f"LuaTools: Updated unsteam.ini with appid {appid}, steam_id={steam_id or 'unknown'}, player_name={player_name or 'unknown'}"
            )
        else:
            logger.log("LuaTools: unsteam.ini already up to date")
