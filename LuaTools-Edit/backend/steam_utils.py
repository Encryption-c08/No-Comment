...
def get_primary_steam_user_info() -> Dict[str, str]:
    ...
    try:
        ...
        if not os.path.exists(login_path):
            logger.warn(f"LuaTools: loginusers.vdf not found at {login_path}")
            return {}
        ...
        if not isinstance(users, dict):
            logger.warn("LuaTools: loginusers.vdf parsed without users")
            return {}
        ...
        if not selected_user or not selected_id:
            logger.warn("LuaTools: No usable user entries found in loginusers.vdf")
            return {}
        ...
        logger.warn(f"LuaTools: Failed to read loginusers.vdf: {exc}")
        return {}
