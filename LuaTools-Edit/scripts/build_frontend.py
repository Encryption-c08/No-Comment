...
def build_luatools_frontend() -> None:
    project_root = Path(__file__).resolve().parent.parent
    parts_dir = project_root / "public" / "src" / "luatools"
    output_file = project_root / "public" / "modules" / "luatools.app.js"
    ...
    wrapped_sections = [
        ...
        "    'use strict';",
        "",
        "    if (window.__LuaToolsAppLoaded) {",
        "        return;",
        "    }",
        "    window.__LuaToolsAppLoaded = true;",
        "",
        "\n\n".join(combined),
        "",
        "})();",
